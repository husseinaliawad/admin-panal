...
// إنشاء مجلد للرفع
$uploadDir = 'upload-ready';
if (!is_dir($uploadDir)) {
    mkdir($uploadDir, 0755, true);
    echo "<div class='info'>📁 تم إنشاء مجلد: $uploadDir</div>";
}
...
foreach ($imageFolders as $folder) {
    $sourceDir = "public/$folder";
    $destDir = "$uploadDir/public/$folder";
    
    if (!is_dir($destDir)) {
        mkdir($destDir, 0755, true);
        echo "<div class='info'>📁 تم إنشاء مجلد: $destDir</div>";
    }
...
echo "<ol>";
echo "<li>اختبر النظام محلياً باستخدام test-images-final.php</li>";
echo "<li>إذا عمل محلياً، ارفع محتوى مجلد upload-ready للخادم</li>";
