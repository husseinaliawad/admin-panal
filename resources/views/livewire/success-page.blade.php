                    ...
                    <div class="flex items-center justify-center w-full pb-6 space-x-4 md:justify-start">
                        <div class="flex flex-col items-start justify-start space-y-2">
                            <p class="text-lg font-semibold leading-4 text-left text-gray-800 dark:text-gray-400">
                                {{ $order->address->first_name ?? $order->first_name }}
                                {{ $order->address->last_name ?? $order->last_name }}
                            </p>
                            <p class="text-sm leading-4 text-gray-600 dark:text-gray-400">
                                {{ $order->address->street_address ?? $order->street_address }}
                            </p>
                            <p class="text-sm leading-4 text-gray-600 dark:text-gray-400">
                                {{ $order->address->city ?? $order->city }},
                                {{ $order->address->state ?? $order->state }},
                                {{ $order->address->zip_code ?? $order->zip_code }}
                            </p>
